...
From the combined mechanical, electrical, and code design, we were unfortunately only able to implement a reliable system up to checkpoint 2 due to time constraints and unforeseen system issues. One of the first issues that we encountered was that our IR sensor array struggled a lot with the very thin line of the final course track. For larger lines our line sensor array worked smoothly with reasonable PID gains, but with the smaller lines we could never find proper gains that would reliably work. The thin lines were problematic for the array, because we ordered a particularly narrow array which on thin lines caused too much reflectance for the sensor to effectively determine whether there was a line or not. We attempted to add blinders to the array, although this did not really change much. This is because regardless of the blinders, the sensors were too tight to properly detect the edge on the thin lines and identify the difference between the black and white. A non-functioning line sensor in this case, made us unable to reliably test the rest of the code because the Romi would never reach that part of the track. We were therefore unable to determine if the code could fully work with the proper gains, but in theory it should. Another issue we encountered was that our Romi would simply brick at times for seemingly no reason and require 4 to 5 reflashings of the firmware. Ultimately if we were to change anything, we would have certainly gone with a larger IR sensor array that was much more spread out to avoid reflectance issues. Also, if we had more time to change things last minute, we would have most likely attempted a hard coded procedure for the line follow instead of trying to use the dysfunctional line sensor. 
